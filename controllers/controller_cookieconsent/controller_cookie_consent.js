...
controller.send_type = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    var status = "";
    if (data.status === "accept_custom") {
      status = "อนุญาตตามที่เลือก"
    } else if (data.status === "accept_all") {
      status = "อนุญาตทั้งหมด"
    } else {
      status = "ปฏิเสธทั้งหมด"
    }
    conn.query("SELECT  * FROM TB_TR_DOMAINGROUP WHERE id_dg=? ",
      [data.t], (err, select_domaingroup) => {
        conn.query("INSERT INTO TB_TR_PDPA_POLICYLOG SET policylog_cookieconsent=?,policylog_ip=?,policylog_browser=?,policylog_sessionid=?,domaingroup_id=?;",
          [status, clientIp, data.browser, sid, data.t], (err, isnert_pdpa_policylog) => {
            console.log("1", err);
            conn.query("SELECT  * FROM TB_TR_DOMAIN_SETTING_COOKIEPOLICY WHERE domain_id=? ",
              [data.t], (err, SELECT_cookiepolicy) => {
                var id_cookie = Object.values(data.kkk);
                var number_notapprove = id_cookie.toString().replaceAll(",", "")
                conn.query("INSERT INTO TB_TR_PDPA_LOG_SETTING_COOKIEPOLICY SET policy_id=?,domaingroup_id=?,cookiepolicy=? ;",
                  [isnert_pdpa_policylog.insertId, data.t, number_notapprove], (err, domain_style_policy) => {
                    console.log("err insert data cookie api", err);
                    Create_Banner(req, data.t)
                    res.end();
                  });
              });
          });
...
function Create_Banner(req, id) {
  var host = process.env.COOKIE_DOMAIN
  req.getConnection((err, conn) => {
    conn.query("SELECT * FROM  TB_TR_DOMAIN_SETTING_COOKIEPOLICY as dsc LEFT JOIN TB_TR_COOKIEPOLICY AS c ON dsc.cookiepolicy_id=c.id_cp  where dsc.domain_id=? AND check_show=1 AND id_status!=4;",
      [id], (err, domain_setting_cookiepolicy) => {
        conn.query("SELECT * FROM TB_TR_DOMAIN_STYLE_POLICY as dsp  LEFT JOIN TB_TR_STYLES as st ON dsp.style_id=st.id_st LEFT JOIN TB_TR_DOMAINGROUP as dg ON dsp.domaingroup_id=dg.id_dg where domaingroup_id=?;",
          [id], (err, domain_style_policy) => {
            var cookie_policy = `${host + `/show_slide/` + domain_style_policy[0].doc_id}`;
            var domain = `${domain_style_policy[0].dg_file}.js`
            ...
            if (domain_style_policy[0].nameclass == "light" || domain_style_policy[0].nameclass == null) {
              color_text = "#000"
...
run_consents(saved_consents);
  ...
  alert("ติดตั้ง script ไม่ตรงกับโดเมนที่ลงทะเบียนไว้")
}
})()
`
