...
controller.AnnouncePolicy = (req, res) => {
    if (typeof req.session.userid != "undefined") {
        ...
        req.getConnection((err, conn) => {
            if (req.session.admin == 5) {
                conn.query("SELECT * FROM TB_TR_PDPA_DOCUMENT WHERE  user_id = ? AND type_policy=0 AND doc_action = 0 and type != 3", [user], async (err, check_new_site) => {
                    if (check_new_site.length == 0 || check_new_site.length == null) {
                        controller.CreateDocument(req)
                    }
                });
            }
            conn.query("SELECT * FROM TB_MM_PDPA_DOCUMENT_TYPE;", (err, doc_type) => {
                conn.query("SELECT * FROM TB_TR_PDPA_DOCUMENT WHERE  user_id = ? AND type_policy=0 AND doc_action = 0 and type != 3", [user], (err, main_doc) => {
                    funchistory.funchistory(req, "ประกาศนโยบาย/เเจ้งเตือน", `เข้าสู่เมนู ประกาศนโยบายทางอิเล็คทรอนิค`, req.session.userid)
                    if (main_doc.length < req.session.limit.policy || req.session.limit.policy === -1) {
                        req.session.limit.policy = 'ไม่เต็ม'
                    } else {
                        req.session.limit.policy = 'เต็ม'
                    }
                    res.render("policy/announce/announce_policy", {
                        doc_type,
                        session: req.session,
                    });
                });
...
controller.AnnouncePolicylist = (req, res) => {
    if (typeof req.session.userid != "undefined") {
        var host = sethost(req);
        var user = '';
        if (req.session.acc_id_control) {
        ...
        req.getConnection((err, conn) => {
            conn.query("SELECT *,DATE_FORMAT(duc.doc_date_create, '%d/%m/%Y %H:%i:%s' ) as date_create FROM TB_TR_PDPA_DOCUMENT as duc LEFT JOIN TB_TR_ACCOUNT as acc ON duc.user_id=acc.acc_id  LEFT JOIN TB_MM_PDPA_DOCUMENT_TYPE as ductype ON duc.doc_type_id=ductype.doc_type_id WHERE type_policy=0 AND doc_action=0 AND duc.doc_type_id=217 AND duc.user_id=?  ORDER BY  ductype.doc_type_id ASC;", [user], (err, announce_policy) => {
                conn.query("SELECT * FROM TB_MM_PDPA_DOCUMENT_TYPE", async (_, document_types) => {
                    if (announce_policy.length > 0) {
                        res.send({ 'limit': req.session.limit.policy, announce_policy, document_types, host })
                    } else {
                        announce_policy = 'ไม่มีข้อมูล'
                        res.send({ 'limit': req.session.limit.policy, announce_policy, document_types, host })
                    }
                });
            });
...
controller.AnnouncePolicyAdd = (req, res) => {
    if (typeof req.session.userid == "undefined") {
        ...
        req.getConnection((err, conn) => {
            conn.query("INSERT INTO TB_TR_PDPA_DOCUMENT SET doc_name = ?, doc_type_id = ?, doc_remark = ?, doc_date_create = Now(), doc_consent_status = ?,user_id = ? , type = 4,type_policy=0,check_doc = 0 ;",
                [data.doc_name, data.doc_type_id, data.doc_remark, data.doc_consent_status, req.session.userid], (err, InsertDocument) => {
...
controller.AnnouncePolicyDeleted = (req, res) => {
    if (typeof req.session.userid == 'undefined') {
        res.redirect('/');
    } else {
                        ...
                        conn.query("SELECT * FROM  TB_TR_PDPA_DOCUMENT as d LEFT JOIN TB_TR_PDPA_DOCUMENT_LOG as dl on dl.doc_id = d.doc_id WHERE d.doc_id = ?", [data_id], (err, doc) => {
                            conn.query("UPDATE TB_TR_PDPA_DOCUMENT SET doc_action = 1 WHERE doc_id = ?", [data_id], (err, du) => {
                                conn.query('SELECT DATE_FORMAT(NOW(), "%Y-%m-%d %H:%i:%s") as new;', (err, date) => {
                                    conn.query("INSERT INTO TB_TR_PDPA_DOCUMENT_LOG SET doc_id = ?, log_date = now(), log_action = 5, log_detail = 'ลบเอกสาร',user_id = ?;", [doc[0].doc_id, req.session.userid], (err, logdoc) => {
                                        res.redirect('/announce-policy');
                                    })
                                })
                            })
...
controller.AnnouncePolicyUpdate = (req, res) => {
    if (typeof req.session.userid == 'undefined') {
        res.redirect('/');
    } else {
        // const user = req.session.userid;
...
controller.AnnouncePolicySendMail = (req, res) => {
    if (typeof req.session.userid == "undefined") {
        res.redirect("/");
    } else {
        var user = '';
...
controller.AnnouncePolicyCopy = (req, res) => {
    if (typeof req.session.userid == 'undefined') {
        res.redirect('/');
    } else {
        // const user = req.session.userid;
...
controller.AnnouncePolicySearchType = (req, res) => {
    if (typeof req.session.userid != "undefined") {
        var host = sethost(req);
        req.getConnection((err, conn) => {
            var user = '';
            ...
            if (req.body.id == 'all') {
                if (req.body.value == 'all') {
                    QuerySelect = `SELECT *,DATE_FORMAT(duc.doc_date_create, '%d/%m/%Y %H:%i:%s' ) as date_create FROM TB_TR_PDPA_DOCUMENT as duc LEFT JOIN TB_TR_ACCOUNT as acc ON duc.user_id=acc.acc_id  LEFT JOIN TB_MM_PDPA_DOCUMENT_TYPE as ductype ON duc.doc_type_id=ductype.doc_type_id WHERE type_policy=0 AND doc_action=0 AND ductype.doc_type_id=217`
                } else {
                    QuerySelect = `SELECT *,DATE_FORMAT(duc.doc_date_create, '%d/%m/%Y %H:%i:%s' ) as date_create FROM TB_TR_PDPA_DOCUMENT as duc LEFT JOIN TB_TR_ACCOUNT as acc ON duc.user_id=acc.acc_id  LEFT JOIN TB_MM_PDPA_DOCUMENT_TYPE as ductype ON duc.doc_type_id=ductype.doc_type_id WHERE type_policy=0 AND doc_action=0 AND ductype.doc_type_id=217 AND duc.doc_name LIKE '%${req.body.value}%'`
                }
                conn.query(QuerySelect, (err, announce_policy) => {
                    conn.query("SELECT * FROM TB_MM_PDPA_DOCUMENT_TYPE", (_, document_types) => {
                        if (announce_policy.length > 0) {
                            res.send({ announce_policy, document_types, host })
                        } else {
                            announce_policy = 'ไม่มีข้อมูล'
                            res.send({ announce_policy, document_types, host })
                        }
                ...
                if (req.body.value == 'all') { // กรณีไม่ได้กรอกข้อมที่ต้องการต้องค้น (input)
                    QuerySelect = `SELECT *,DATE_FORMAT(duc.doc_date_create, '%d/%m/%Y %H:%i:%s' ) as date_create FROM TB_TR_PDPA_DOCUMENT as duc LEFT JOIN TB_TR_ACCOUNT as acc ON duc.user_id=acc.acc_id  LEFT JOIN TB_MM_PDPA_DOCUMENT_TYPE as ductype ON duc.doc_type_id=ductype.doc_type_id WHERE type_policy=0 AND doc_action=0 AND ductype.doc_type_id=${req.body.id}`
                } else {
                    QuerySelect = `SELECT *,DATE_FORMAT(duc.doc_date_create, '%d/%m/%Y %H:%i:%s' ) as date_create FROM TB_TR_PDPA_DOCUMENT as duc LEFT JOIN TB_TR_ACCOUNT as acc ON duc.user_id=acc.acc_id  LEFT JOIN TB_MM_PDPA_DOCUMENT_TYPE as ductype ON duc.doc_type_id=ductype.doc_type_id WHERE type_policy=0 AND doc_action=0 AND ductype.doc_type_id=${req.body.id} AND duc.doc_name LIKE '%${req.body.value}%'`
                }
                conn.query(QuerySelect, (err, announce_policy) => {
                    conn.query("SELECT * FROM TB_MM_PDPA_DOCUMENT_TYPE", (_, document_types) => {
                        if (announce_policy.length > 0) {
                ...
                conn.query("SELECT * FROM TB_TR_PDPA_DOCTEMP_PAGE AS dp LEFT JOIN TB_TR_PDPA_DOCTEMP AS d ON dp.doc_id = d.doc_id WHERE d.doc_id = ? ",
                    [main_doc[i].doc_id], (err, main_doc_page) => {
                        conn.query("INSERT INTO TB_TR_PDPA_DOCUMENT SET doc_type_id = ?,doc_name = ?,doc_date_create = NOW(),user_id =?,doc_status =1,doc_remark=?,doc_action=?,doc_consent_status=?,type_policy=0,check_doc=?,type=0",
                            [main_doc_page[0].doc_type_id, main_doc_page[0].doc_name, user, main_doc[i].doc_remark, main_doc[i].doc_action, main_doc[i].doc_consent_status, main_doc[i].check_doc,],
                            (err, add_main_doc) => {
                                conn.query("INSERT INTO TB_TR_PDPA_DOCUMENT_LOG SET doc_id = ?, log_date = NOW(), log_action = 8, log_detail = ? ,user_id = ? ",
                                    [add_main_doc.insertId, "สร้างเอกสารต้นแบบ เป็น ใช้งาน", user,], (err, doc_default) => {
...
// controller.check_polic_limit = async (req, res) => {
//     req.getConnection((err, conn) => {
//         conn.query("SELECT * FROM TB_TR_PDPA_DOCUMENT WHERE  user_id = ? AND type_policy=0 AND doc_action = 0 and type != 3", [req.session.userid], (err, main_doc) => {
//             if (req.session.limit.policy == main_doc.length) {
//                 req.session.limit.policy = 'เต็ม'
//             } else {
//                 req.session.limit.policy = 'ไม่เต็ม'
//             }
