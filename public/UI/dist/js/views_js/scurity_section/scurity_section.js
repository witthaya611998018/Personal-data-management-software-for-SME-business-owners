...
function delete_Measures(data) {
    console.log($(data).closest("tr").find("td").eq(1).text().trim());
    let input = (document.getElementById('Delete-Measures')).getElementsByTagName('input')
    let label = (document.getElementById('Delete-Measures')).getElementsByTagName('label')
    
    input[0].value = $(data).closest("tr").find("td").eq(0).attr('id')
    label[0].innerHTML = $(data).closest("tr").find("td").eq(2).text().trim()
    
};

function delete_approach_Information(data) {
    console.log($(data).closest("tr").find("td").eq(1).text().trim());
    let input = (document.getElementById('Delete-approach_Information')).getElementsByTagName('input')
    let label = (document.getElementById('Delete-approach_Information')).getElementsByTagName('label')
...
function delete_approach_Network(data) {
    console.log($(data).closest("tr").find("td").eq(1).text().trim());
    let input = (document.getElementById('Delete-approach_Network')).getElementsByTagName('input')
    let label = (document.getElementById('Delete-approach_Network')).getElementsByTagName('label')
...
function delete_specific(data) {
    console.log($(data).closest("tr").find("td").eq(1).text().trim());
    let input = (document.getElementById('Delete-specific')).getElementsByTagName('input')
    let label = (document.getElementById('Delete-specific')).getElementsByTagName('label')
    ...
    if (data == 'ไม่มีข้อมูล') {
        ...
        function buildTable() {
            ...
            for (var i in myList) {
                ...
                let delete_trash = '<a  class="text-danger" data-bs-toggle="modal" data-bs-target="#Delete-Measures" onclick="delete_Measures(this)"><i class="fas fa-trash-alt fa-2x"></i></a>'
                var row = `<tr>
                <td id="${myList[i].measures_id}">${myList[i].no}</td>
                <td>${myList[i].measures_type}</td>           
                <td>${myList[i].measures_section_name}</td>
                <td>${doc_show}</td>
                <td>${myList[i].measures_detail}</td>
                <td>  <a class="text-info" href="/General-Measures/details/${myList[i].measures_id}"><i class=" fas fa-file-alt fa-2x"></i></a></td>
...
async function DatabTable_Risk_Based_Approach_Information(Information, doc) {
    if (Information == 'ไม่มีข้อมูล') {
        ...
        function pageButtons(pages) {
            ...
            $('.page').on('click', function () {
                $('#tbody-Risk-based-Approach-Information').empty()
                state.page = Number($(this).val())
                buildTable()
            })
        };
        function buildTable() {
            ...
            for (var i in myList) {
                ...
                let delete_trash = '<a  class="text-danger" data-bs-toggle="modal" data-bs-target="#Delete-approach_Information" onclick="delete_approach_Information(this)"><i class="fas fa-trash-alt fa-2x"></i></a>'
                var row = `<tr>
                <td id="${myList[i].measures_id}">${myList[i].no}</td>
                <td>${myList[i].measures_section_name}</td>           
...
async function DatabTable_Risk_Based_Approach_Network(Network, doc) {
    if (Network == 'ไม่มีข้อมูล') {
        ...
        function pageButtons(pages) {
            ...
            $('.page').on('click', function () {
                $('#tbody-Risk-based-Approach-Network').empty()
                state.page = Number($(this).val())
                buildTable()
            })
        };
        function buildTable() {
            ...
            for (var i in myList) {
                ...
                let delete_trash = '<a  class="text-danger" data-bs-toggle="modal" data-bs-target="#Delete-approach_Network" onclick="delete_approach_Network(this)"><i class="fas fa-trash-alt fa-2x"></i></a>'
                var row = `<tr>
                <td id="${myList[i].measures_id}">${myList[i].no}</td>
                <td>${myList[i].measures_section_name}</td>           
                <td>${myList[i].assets_name}</td>
...
async function DatabTable_Risk_Based_Papa_Specific(specific, doc) {
    if (specific == 'ไม่มีข้อมูล') {
        ...
        function pageButtons(pages) {
            ...
            $('.page').on('click', function () {
                $('#tbody-Risk-based-Specific-Measures').empty()
                state.page = Number($(this).val())
                buildTable()
            })
        };
        function buildTable() {
            ...
            for (var i in myList) {
                ...
                let delete_trash = '<a  class="text-danger" data-bs-toggle="modal" data-bs-target="#Delete-specific" onclick="delete_specific(this)"><i class="fas fa-trash-alt fa-2x"></i></a>'
                var row = `<tr>
                <td id="${myList[i].measures_id}">${myList[i].no}</td>
