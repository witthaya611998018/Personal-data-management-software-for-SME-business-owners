...
function get_data() {
    $.ajax({
        ...
        success: async function (result) {
            ...
            await DataTable_Assets(result.assets,result.check_e_d)
        },
        error: function (e) {
            console.log(e);
        }
...
async function DataTable_Assets(data,check_2) {
    if (data == 'ไม่มีข้อมูล') {
        ...
        function buildTable() {
            ...
            var data = pagination(state.querySet, state.page, state.rows,state.check_data)
            var myList = data.querySet
            var ch = data.check_data
            var show = [];
            for (var i in myList) {
                let delete_trash = '<i class="fas fa-trash-alt fa-2x text-secondary"></i>'
                ...
                if (myList[i].status == 2) {
                    delete_trash = '<i class="fas fa-trash-alt fa-2x text-secondary"></i>'
                    edit_trash = `
                    <a  href="#Edit-Assets" data-bs-toggle="modal" onclick="edite_assets(this)" class="text-warning">
                            <i class="fas fa-pencil-alt fa-2x"></i>
                            </a>
                    `
                }else{
                ...
                let device_c_name = '';
                if (myList[i].status == 3) {
                    device_c_name = '<p style="color: red;display: inline-block;">(อุปกรณ์นี้ถูกลบแล้ว)</p>'
                }
                var row = `<tr>
                        ...
                        <td>
                        อ้างอิงจาก: ${myList[i].old_name}&nbsp;${device_c_name}
                        </td>         
                        <td>
                            ${edit_trash}
                        </td>
            ...
            document.querySelector("#show-depth_2").innerHTML = show[0];  //  แสดงถึง row เเรกของหน้า 
            document.querySelector("#to-show-depth_2").innerHTML = show[show.length - 1];  //  แสดงถึง row สุดท้ายของหน้า
            document.querySelector("#show-all-depth_2").innerHTML = state.querySet.length;  //  แสดงถึงจำนวนข้อมูลทั้งหมดของหน้า
            pageButtons(data.pages)
        }
    }
}
