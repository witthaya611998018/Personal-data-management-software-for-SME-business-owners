$(function () {
    ...
    async function ready_datanull() {
        ...
        var thead =
            '<tr>' +
            '<th>ลำดับ</th>' +
            '<th>ชื่อ Pattern (กิจกรรม)</th>' +
            '<th>ผู้สร้าง</th>' +
            '<th>วันที่สร้าง</th>' +
            '<th>เอกสาร</th>' +
            '<th>ข้อมูลส่วนบุคคล</th>' +
            '<th>ระยะเวลาประมวลผล</th>' +
    ...
    document.getElementById("button-addon2").addEventListener("click", () => {
        ...
        $.ajax({
            ...
            url: '/api/search/report/classification',
    ...
    document.getElementById("search_date").addEventListener("click", () => {
        ...
        $.ajax({
            type: "post",
            contentType: "application/json",
            url: '/api/report/classification/search_date',
            data: JSON.stringify(data),
            dataType: 'json',
            success: async function (result) {
                if (result == "ไม่มีข้อมูล") {
    ...
    document.getElementById("csv").addEventListener("click", () => {
        ...
        for (let i = 0; i < csvFileData.length; i++) {
            ...
            csvData.push(
                [csvFileData[i].no,
                csvFileData[i].classify_name,
                csvFileData[i].user,
                csvFileData[i].day_classify_create,
                csvFileData[i].list_policy,
                csvFileData[i].list_data,
                csvFileData[i].classify_period_process,
                csvFileData[i].pattern_processing_base_name,
                csvFileData[i].Datamask
                ]
            )
        }
        var type = "";
        ...
        var csv = `ลำดับ,ชื่อ Pattern (กิจกรรม),ผู้สร้าง,วันที่สร้าง,เอกสาร,ข้อมูลส่วนบุคคล,ระยะเวลาประมวลผล,ฐานการประมวลผล,Datamask\n`;
        ...
        hiddenElement.download = 'ข้อมูลตาม Classification.csv';
        hiddenElement.click();
    });
    ...
    document.getElementById("excel").addEventListener("click", () => {
        ...
        for (let i = 0; i < csvFileData.length; i++) {
            ...
            var object_exce = {
                ...
                ชื่อPattern: csvFileData[i].classify_name ,
                ผู้สร้าง: csvFileData[i].user,
                วันที่สร้าง: csvFileData[i].day_classify_create ,
                เอกสาร: csvFileData[i].list_policy ,
                ข้อมูลส่วนบุคคล: csvFileData[i].list_data ,
                ระยะเวลาประมวลผล: csvFileData[i].classify_period_process ,
                ฐานการประมวลผล: csvFileData[i].pattern_processing_base_name  ,
                Datamask: csvFileData[i].Datamask 
            }
        ...
        XLSX.writeFile(wb, 'ข้อมูลตาม Classification.xlsx');
    });
    ...
    document.getElementById('pdf').addEventListener('click', function (event) {
        ...
        for (let i = 0; i < csvFileData.length; i++) {
            ...
            content +=
                ...
                '</td><td>' + csvFileData[i].classify_name +
                '</td><td>' + csvFileData[i].user +
                '</td><td>' + csvFileData[i].day_classify_create +
                '</td><td>' + csvFileData[i].list_policy +
                '</td><td>' + csvFileData[i].list_data +
                '</td><td>' + csvFileData[i].classify_period_process +
                '</td><td>' + csvFileData[i].pattern_processing_base_name +
                '</td><td>' + csvFileData[i].Datamask +
            '</td></tr>'
        }
        var footer_content = `</tbody></table>`
        myWindow.document.write("ข้อมูลตาม Classification", header_content + content + footer_content)
    ...
    async function Tabeldata_ajax(data) {
        console.log(data);
        var classify = data.classify_data
        var data_classify = [];
        csvFileData = [];
        for (var i = 0; i < classify.length; i++) {
            dm = ''
            if (classify[i].classify.classify_type_data_in_event_personal_datamark_check == 1) {
                if (classify[i].classify.classify_type_data_in_event_personal_datamark == 0) {
                    dm = 'Pseudonymized หัวข้อมูลอย่างเดียว [ จำนวนตัวอักษรที่แสดง : '+classify[i].classify.classify_type_data_in_event_personal_datamark_total+ ' ])'
                } else if (classify[i].classify.classify_type_data_in_event_personal_datamark == 1) {
                    dm = 'Pseudonymized (หัวและท้ายข้อมูล [ จำนวนตัวอักษรที่แสดง: '+classify[i].classify.classify_type_data_in_event_personal_datamark_total+ ' ])'
                } else if (classify[i].classify.classify_type_data_in_event_personal_datamark == 2) {
                    dm = 'Pseudonymized (ท้ายข้อมูลอย่างเดียว [ จำนวนตัวอักษรที่แสดง  '+classify[i].classify.classify_type_data_in_event_personal_datamark_total+ ' ])'
                } else if (classify[i].classify.classify_type_data_in_event_personal_datamark == 3) {
                    dm = 'Anonymized'
                }
            } else {
                dm = 'ไม่มี'
            ...
            data_classify.push({
                "no": (i + 1),
                "classify_name": classify[i].classify.classify_name,
                "user": classify[i].classify.name,
                "day_classify_create": classify[i].classify.day_classify_create,
                "list_policy": classify[i].policy,
                "list_data": classify[i].pdpa_data,
                "classify_period_process": classify[i].classify.classify_period_process,
                "classify_period_end": classify[i].classify.classify_period_end,
                "pattern_processing_base_name": classify[i].classify.pattern_processing_base_name,
                "Datamask": dm
            });
            ...
            csvFileData.push({
                ...
                "day_classify_create": classify[i].classify.day_classify_create,
                "list_policy": classify[i].policy,
                "list_data": classify[i].pdpa_data.toString().replace(/,/g, ""),
                "classify_period_process": classify[i].classify.classify_period_process,
                "classify_period_end": classify[i].classify.classify_period_end,
                "pattern_processing_base_name": classify[i].classify.pattern_processing_base_name,
                "Datamask": dm
        ...
        function buildTable() {
            ...
            for (var i in myList) {
                ...
                var row = '<tr>' +
                    '<td>' + myList[i].no + '</td>' +
                    '<td>' + myList[i].classify_name + '</td>' +
                    '<td>' + myList[i].user + '</td>' +
                    '<td>' + myList[i].day_classify_create + '</td>' +
                    '<td>' + myList[i].list_policy + '</td>' +
                    '<td>' + myList[i].list_data.toString().replace(/,/g, "") + '</td>' +
                    '<td>' + myList[i].classify_period_process + '</td>' +
                    '<td>' + myList[i].pattern_processing_base_name + '</td>' +
                    '<td>' + myList[i].Datamask + '</td>' +
                    '</tr>';
