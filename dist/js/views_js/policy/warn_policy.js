...
let GetPolicyWarn = () => {
    $.ajax({
        type: 'GET',
        url: '/api/get-data/warn-policy',
        success: async function (result) {
            types_policy = []
            host = result.host
            if (result.warn_policy == "ไม่มีข้อมูล") {
                DataNull()
            } else {
                await DatabTable(result.warn_policy)
...
document.getElementById('search_type_policy').addEventListener('click', () => {
    var value = (document.getElementById('input_policy_name').value).trim()
    let data = {
    ...
    $.ajax({
        ...
        url: '/api/warn-policy/search-type',
...
let DataNull = () => {
    ...
    content += '</table>'
    $('#DataTable-Warn-Policy').append(content);
...
function DeleteWarnPolicy(data) {
    var name_policy = $(data).closest("tr").find("td").eq(1).text()
...
// document.getElementById('search_policy_name').addEventListener('click', () => {
//     let policy_name = (document.getElementById('input_policy_name').value).trim()
//     if (policy_name == "") {
//         policy_name = 'all'
//     }
//     var data = { 'policy_name': policy_name }
//     $.ajax({
//         type: "post",
//         contentType: "application/json",
//         url: '/api/warn-policy/search-policy-name',
//         data: JSON.stringify(data),
//         dataType: 'json',
//         success: async function (result) {
//             console.log(result);
    ...
    function buildTable() {
        ...
        `,
            ...
            var row = `<tr><td>` + myList[i].no +
                ...
                `</td><td>
                    <a id="${myList[i].doc_id}" doc_remark="${myList[i].doc_remark}" doc_status="${myList[i].doc_status}" 
                    doc_type_id="${myList[i].doc_type_id}" onclick="EditeWarnPolicy(this)"
                    class="text-warning" data-bs-toggle="modal"
                    data-bs-target="#edit_announce_policy"><i class="fas fa-pencil-alt fa-2x"></i>
                     </a>` +
                '</td><td>' + myList[i].date_create +
                '</td><td>' + myList[i].firstname +
                `</td><td>` + myList[i].date_create +
                '</td><td>' + '-' +
                `</td><td>` + myList[i].doc_type_name +
                `</td><td>
                <a id="${myList[i].doc_id}"  onclick="DeleteWarnPolicy(this)"
