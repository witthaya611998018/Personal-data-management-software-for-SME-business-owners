...
function get_data() {
    ...
    $.ajax({
        type: 'GET',
        url: '/api/email-owner-personal/list',
        success: async function (result) {
            if (result.pdpa_email_board == "ไม่มีข้อมูล") {
                ...
                await Data_Table(result.pdpa_email_board)
                if (result.email_board === -1 || result.pdpa_email_board.length < result.email_board) {
                    document.getElementById('limit').innerHTML = `
                        <a class="btn btn-success" id="inbox-email" href="/email-owner-personal/inbox">
...
async function Data_Table(data) {
...
function resend_mail(id) {
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                type: 'GET',
                url: '/api/resend-email-owner-personal/' + id,
                success: async function (result) {
                    if (result == "ไม่มีข้อมูล") {
                        data_null()
                    } else {
                        $('#table-body').empty()
                        await Data_Table(result)
...
document.getElementById('button-search').addEventListener('click', () => {
    ...
    $.ajax({
        ...
        url: '/api/email-owner-personal/search/text',
